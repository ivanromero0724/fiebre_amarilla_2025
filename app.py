...
with col1:
    fig1 = px.pie(datos, names="1_MUNICIPIO", title="Distribución por Municipio",color_discrete_sequence=px.colors.qualitative.Safe)
    fig1.update_layout(title={'x': 0.5, 'xanchor': 'center'})
    st.plotly_chart(fig1, use_container_width=True)

with col2:
    fig2 = px.pie(datos, names="2_AREA", title="Distribución por Área",color_discrete_sequence=px.colors.qualitative.Safe)
    fig2.update_layout(title={'x': 0.5, 'xanchor': 'center'})
    st.plotly_chart(fig2, use_container_width=True)

with col3:
    fig3 = px.pie(datos, names="6_VIVIENDA_EFECTIVA_", title="Viviendas Efectivas vs No Efectivas",color_discrete_sequence=[px.colors.qualitative.Safe[3], px.colors.qualitative.Safe[1]])
...
with col2:  # Coloca la tabla en la columna central
    st.markdown("<h5 style='text-align: center;'>Resumen de Viviendas por Municipio</h5>", unsafe_allow_html=True)
